...
const PlaceOrder = () => {
  ...
  const { navigate, backendUrl, token, cartItems, setCartItems, getCartAmount, getCartItems, delivery_fee, products } = useContext(ShopContext);
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      for (const items in cartItems) {
        for (const item in cartItems[items]) {
          if (cartItems[items][item] > 0) {
            const itemInfo = structuredClone(products.find(product => product._id.toString() === items))
            if (itemInfo) {
              itemInfo.size = item
      ...
      switch (method) {
        ...
        case 'stripe':
          console.log("Token before Stripe request:", token);
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col sm:flex-row justify-between  gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
      {/*----------- Left side ------------------*/}
      <div className='flex flex-col gap-4 w-ful sm:max-w-120'>
        ...
        <input required onChange={onChangeHandler} name='email' value={formData.email} className='border border-gray-300 rounded py-1.5 px-3 w-full' type="emaill" placeholder='Email address' />
      ...
      <div className='mt-8'>
        ...
        <div className='mt-12'>
          ...
          <div className='flex flex-col gap-3 lg:flex-row'>
            ...
            <div onClick={() => setMethod('stripe')} className=' flex items-center gap- border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'stripe' ? 'bg-green-500' : ''}`}></p>
              <img className='h-5 mx-4' src={assets.stripe_logo} alt="" />
            </div>
            <div onClick={() => setMethod('razorpay')} className=' flex items-center gap- border p-2 px-3 cursor-pointer'>
              <p className={`min-w-3.5 h-3.5 border rounded-full ${method === 'razorpay' ? 'bg-green-500' : ''}`}></p>
              <img className='h-5 mx-4' src={assets.razorpay_logo} alt="" />
            </div>
            <div onClick={() => setMethod('cod')} className=' flex items-center gap- border p-2 px-3 cursor-pointer'>
